...
retrieve_data <- function(symbol="BTC-USDT", time_period="1day",
  ...
  apiurl <- stringr::str_glue('https://api.kucoin.com/api/v1/market/candles?type={time_period}&symbol={symbol}&startAt={start_date_unix}&endAt={end_date_unix}')

  # Get Raw Data in Json Format
  res = httr::GET(apiurl)
  rawdata <- jsonlite::fromJSON(rawToChar(res$content))$data
  ...
  data$Date <- anytime::anydate(as.numeric(data$Date))
